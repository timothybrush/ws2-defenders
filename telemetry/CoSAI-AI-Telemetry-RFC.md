...
Go to the **classification summary** ([§4.5](#45-classification-summary), the single table listing every field by component and tier) and treat the **applicable subset of the MUST column** as the baseline each AI deployment should meet. The three tiers are **MUST**, **SHOULD**, and **MAY**, used in the RFC 2119 sense and defined in [§4.2](#42-classification-legend). The catalogue contains 48 MUST fields; a deployment's baseline consists of those whose defining component, operation, or event exists in that deployment. This is the artifact to take into an engineering plan or a budget discussion.
...
The full MUST catalogue contains 48 fields, which is more than most full-stack deployments can instrument at once. Within a deployment's applicable subset, adoption has an order.
...
- **Tool Definition Digest is SHOULD on the modality gate.** The evidence argument that once held it there is closed, and by two instances of different kinds: `TA-14` is a definition approved and then mutated, where trust bound to a tool's *name* survives a change to its command, and `TA-15` is a definition that is malicious as published, the injection carried in the tool's own description. The first is why the digest is compared against an approved baseline; the second is why it is taken **at invocation** rather than at registration. `TA-16` adds the version-level case, a package rug-pulled after adoption. What remains is the modality gate: a deployment whose tools are first-party and static has no drift to detect. **MUST for deployments consuming third-party or dynamically-discovered tools.**
- **MCP Server Identity & Primitive is SHOULD on the modality gate alone.** The corpus carries six MCP incidents (`TA-11` to `TA-16`), four of which this field cites directly, so the evidence gate is cleared several times over. `TA-16` is the sharpest: the server's declared contract never changed, only its published **version** did, so name alone would not have distinguished the safe release from the malicious one. What holds it below MUST is the modality gate, since a deployment running no MCP surface has nothing to record. **MUST wherever MCP is in use**; server name and version are as cheap as Tool Name and should be adopted first.
