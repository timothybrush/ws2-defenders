...
OTel GenAI attributes (gen_ai.*) are used for agent identity, tool invocation,
retrieval, and evaluation where OTel defines standard conventions.
AITF extensions use short namespaces without the aitf. prefix
(e.g., cost.*, security.*, mcp.*, agent.step.*).
"""
...
class AgentAttributes:
    ...
    # Core agent attributes (name, id, description, version now in GenAIAttributes)
    TYPE = "agent.type"
    FRAMEWORK = "agent.framework"

    # Session attributes (session.id now gen_ai.conversation.id in GenAIAttributes)
    ...
    class PeerVerificationResult:
        ...
        NOT_ATTEMPTED is a configuration finding: no verification was tried.
        UNVERIFIED means verification ran but produced no evaluable answer.
        REFUSED means the peer declined inspection -- a positive act, not silence.
        """
...
class MCPAttributes:
    ...
    # Tool attributes (name now gen_ai.tool.name, input now gen_ai.tool.call.arguments,
    # output now gen_ai.tool.call.result in GenAIAttributes)
    ...
    RESPONSE_ERROR = "mcp.tool.response_error"
    ...
    class Primitive:
        ...
        The primitive class names the kind of authority the server exercised.
        SAMPLING and ELICITATION are the two that invert the direction of
        control: the server drives the client's model or its user.
        """
...
class RAGAttributes:
    ...
    # QUERY now gen_ai.retrieval.query.text in GenAIAttributes
    QUERY_EMBEDDING_MODEL = "rag.query.embedding_model"
    QUERY_EMBEDDING_DIMENSIONS = "rag.query.embedding_dimensions"

    # Retrieval attributes
    # RETRIEVE_DATABASE now gen_ai.data_source.id in GenAIAttributes
    RETRIEVE_INDEX = "rag.retrieve.index"
    # RETRIEVE_TOP_K now gen_ai.request.top_k in GenAIAttributes
...
class SecurityAttributes:
    ...
    class TaintScope:
        ...
        The scope states how far a taint label carries. A label scoped to
        MESSAGE does not constrain a later turn; one scoped to SESSION does.
        """
    ...
    class EnforcementFailureMode:
        """Values for security.enforcement.failure_mode (RFC v0.4 gap closure).

        FAIL_OPEN means the operation proceeded without the control. It is the
        value that turns a missing denial into a known blind spot rather than
        evidence that the operation was permitted.
        """
    ...
    class AttributeSource:
        ...
        Per RFC section 16, an unmarked security attribute must be read as
        SELF_ASSERTED.
        """
...
class AIBOMAttributes:
    ...
    BOM_ID = "ai_bom.id"
    BOM_VERSION = "ai_bom.version"
    BOM_FORMAT = "ai_bom.format"
    BOM_GENERATED_AT = "ai_bom.generated_at"
    BOM_GENERATOR = "ai_bom.generator"
    BOM_GENERATOR_VERSION = "ai_bom.generator_version"

    # Component attributes
    COMPONENT_TYPE = "ai_bom.component.type"
    COMPONENT_NAME = "ai_bom.component.name"
    COMPONENT_VERSION = "ai_bom.component.version"
    COMPONENT_PROVIDER = "ai_bom.component.provider"
    COMPONENT_HASH = "ai_bom.component.hash"
    COMPONENT_LICENSE = "ai_bom.component.license"
    COMPONENT_SOURCE = "ai_bom.component.source"

    # Dependency attributes
    DEPENDENCY_COUNT = "ai_bom.dependency.count"
    DEPENDENCY_DIRECT_COUNT = "ai_bom.dependency.direct_count"
    DEPENDENCY_TRANSITIVE_COUNT = "ai_bom.dependency.transitive_count"

    # Vulnerability attributes
    VULNERABILITY_COUNT = "ai_bom.vulnerability.count"
    VULNERABILITY_CRITICAL = "ai_bom.vulnerability.critical"
    VULNERABILITY_HIGH = "ai_bom.vulnerability.high"

    # Provenance attributes
    PROVENANCE_VERIFIED = "ai_bom.provenance.verified"
    PROVENANCE_SIGNED = "ai_bom.provenance.signed"
    PROVENANCE_SIGNER = "ai_bom.provenance.signer"
...
class MemoryAttributes:
    ...
    class ConfigScope:
        ...
        GLOBAL means content one principal injected is reachable by principals
...
class IdentityAttributes:
    ...
    class ApprovalDecision:
        ...
        BYPASSED records that the operation proceeded without a decision. The
        absence of a decided event MUST NOT be read as a denial.
        """
    ...
    class ApprovalScopeBinding:
        """Values for identity.approval.scope_binding.result (RFC v0.4 gap closure).

        MISMATCH is the consent-substitution signal: what was approved is not
        what was executed. NOT_CHECKED means the binding was never evaluated.
        """
    ...
    class ApprovalStatus:
        ...
        PENDING and EXPIRED both leave the requested authority unresolved.
        Neither may be read as a denial.
        """
...
class AssetInventoryAttributes:
    ...
    class CapabilityChangeSource:
        """Values for asset.capability.change_source (RFC v0.4 gap closure).

        Separates governed change paths from ungoverned ones. RUNTIME_DISCOVERY
        and UNKNOWN indicate capability expansion that no change process saw.
        """
...
class MemorySecurityAttributes:
    """AITF Memory Security semantic convention attributes (extends memory.*)."""
